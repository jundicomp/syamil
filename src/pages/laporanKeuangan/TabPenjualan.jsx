import { useData } from '../../context/DataContext';
import DataTable from '../../components/common/DataTable';

const COLUMNS = [
  { key: 'tanggal', label: 'Tanggal' },
  { key: 'noNota', label: 'No. Nota' },
  { key: 'pelanggan', label: 'Pelanggan' },
  { key: 'total', label: 'Total', type: 'currency', align: 'r' },
  { key: 'dpDibayar', label: 'Diterima (Tunai/Bank)', type: 'currency', align: 'r' },
  { key: 'sisaBayar', label: 'Sisa (Piutang)', type: 'currency', align: 'r' },
  { key: 'status', label: 'Status', type: 'badge' },
];

export default function TabPenjualan() {
  const { data } = useData();
  const totalOmzet = data.penjualan.reduce((s, r) => s + r.total, 0);

  return (
    <DataTable
      title="Laporan Penjualan"
      subtitle={`${data.penjualan.length} transaksi · Total omzet Rp${totalOmzet.toLocaleString('id-ID')}`}
      columns={COLUMNS}
      rows={data.penjualan}
      actions={[]}
      dateKey="tanggal"
      summaryKeys={['total', 'dpDibayar', 'sisaBayar']}
    />
  );
}
