...
export default function ExportButtons({ title, subtitle, columns, rows }) {
  ...
  return (
    <div className="export-group">
      <button className="btn-export" onClick={handleExcel} disabled={rows.length === 0}><Icon name="excel" size={14} /> Excel</button>
      <button className="btn-export" onClick={handlePDF} disabled={rows.length === 0}><Icon name="pdf" size={14} /> PDF</button>
    </div>
  );
}
